...
def crawl_web(seed,max_depth):
    ...
    while tocrawl:
        ...
        if (page not in crawled) and (depth<max_depth):
            for e in get_all_links(get_page(page_url)):
            	union(tocrawl, [[e,depth+1]])
            crawled.append([page_url,depth])
        elif (page not in crawled) and (page[1]==max_depth):
        	crawled.append([page_url,depth])
    crawled_to_ret=[]
    for e in crawled:
    	union(crawled_to_ret,[e[0]])
    print crawled
    return crawled_to_ret
